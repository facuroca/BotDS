...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (cantidad <= 1 || cantidad > 100) {
            return interaction.editReply({ content: 'Debes ingresar un número entre 1 y 99.' });
        }

        await interaction.channel.bulkDelete(cantidad, true).catch(error => {
            console.error(error);
            interaction.editReply({ content: 'Hubo un error al intentar borrar mensajes en este canal.' });
        });
